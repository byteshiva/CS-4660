...
class FifteenPuzzle:
    ...
    def shuffle(self, steps: int) -> Board_Dict:
        ...
        for i in range(steps):
            neighbors: List[Board_Dict] = self.neighbors(puzzle_board)
            # Sort the neighbors by man_dist (high to low).
            sorted_neighbors = sorted(neighbors, key=lambda n: self.man_dist(n), reverse=True)
            # Select randomly one of the two most disordered.
            puzzle_board = choice(sorted_neighbors[:2])
        return puzzle_board
